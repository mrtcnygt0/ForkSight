...
<p align="center">
  <a href="https://chromewebstore.google.com/detail/forksight-%E2%80%94-chess-trainin/nbeldcglpjbmlklbakipohnjcpjmcikd"><img src="https://img.shields.io/badge/Chrome_Web_Store-v3.3.1-f5c542?style=for-the-badge&logo=googlechrome&logoColor=white" alt="Chrome Web Store" /></a>
...
- **PGN castling fix** — `O-O` / `O-O-O` now resolve to UCI arrays (was a bare string that desynced the board and failed later on moves like `Kb1`)

## What's new in v3.3

- **Koçla Öğren** — expanded curriculum (capture lessons, check threat) with Chess.com-style map UI
- **Koçunla Oyna** — match history synced to server; one-tap PGN/analysis from MY COACH
- **Dynamic coach insights** — home card uses real puzzle stats, weakness report, and win-rate trends
- **Draggable dashboard panel** — move the ForkSight panel anywhere on screen
- **PGN analysis fixes** — ambiguous SAN resolution + `[ForkSightUCI]` header for coach games
- Full **Turkish i18n** on the Coach picker page
...
```
extension/     Chrome extension (Manifest V3) — v3.3.1
